...
void FrequalizerAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    g.drawHorizontalLine (
        juce::roundToInt (plotFrame.getY() + 0.25 * plotFrame.getHeight()),
        float (plotFrame.getX()),
        float (plotFrame.getRight()));
    g.drawHorizontalLine (
        juce::roundToInt (plotFrame.getY() + 0.75 * plotFrame.getHeight()),
        float (plotFrame.getX()),
        float (plotFrame.getRight()));

    g.setColour (juce::Colours::silver);
    g.drawFittedText (juce::String (maxDB) + " dB",
                      plotFrame.getX() + 3,
                      plotFrame.getY() + 2,
                      50,
                      14,
    ...
    g.drawFittedText (
        ...
        plotFrame.getX() + 3,
        juce::roundToInt (plotFrame.getY() + 2 + 0.25 * plotFrame.getHeight()),
        50,
        14,
        juce::Justification::left,
        1);
    g.drawFittedText (
        " 0 dB",
        plotFrame.getX() + 3,
        juce::roundToInt (plotFrame.getY() + 2 + 0.5 * plotFrame.getHeight()),
        50,
        14,
        juce::Justification::left,
        1);
    g.drawFittedText (
        juce::String (-maxDB / 2) + " dB",
        plotFrame.getX() + 3,
        juce::roundToInt (plotFrame.getY() + 2 + 0.75 * plotFrame.getHeight()),
        50,
        14,
        juce::Justification::left,
        1);
    ...
    g.drawFittedText (
        "Input", plotFrame.reduced (8), juce::Justification::topRight, 1);
    ...
    for (size_t i = 0; i < freqProcessor.getNumBands(); ++i)
    {
        ...
        g.strokePath (bandEditor->frequencyResponse,
                      juce::PathStrokeType (1.0));
        g.setColour (draggingBand == int (i) ? band->colour
                                             : band->colour.withAlpha (0.3f));
        auto x = juce::roundToInt (
            plotFrame.getX()
            + plotFrame.getWidth()
                  * getPositionForFrequency (float (band->frequency)));
        auto y = juce::roundToInt (
            getPositionForGain (float (band->gain),
                                float (plotFrame.getY()),
                                float (plotFrame.getBottom())));
...
void FrequalizerAudioProcessorEditor::mouseDown (const juce::MouseEvent& e)
{
    ...
    for (int i = 0; i < bandEditors.size(); ++i)
    {
        if (auto* band = freqProcessor.getBand (size_t (i)))
        {
            if (std::abs (
                    plotFrame.getX()
                    + getPositionForFrequency (float (int (band->frequency))
                                               * plotFrame.getWidth())
                    - e.position.getX())
                < clickRadius)
            {
                ...
                contextMenu.showMenuAsync (
                    ...
                    {
                        if (selected > 0)
                            bandEditors.getUnchecked (i)->setType (selected
                                                                   - 1);
                    });
                return;
            }
                        ...
                        < clickRadius)
                    {
                        draggingGain =
                            freqProcessor.getPluginState().getParameter (
                                freqProcessor.getGainParamName (size_t (i)));
...
void FrequalizerAudioProcessorEditor::mouseDrag (const juce::MouseEvent& e)
{
    ...
    {
        auto pos =
            (e.position.getX() - plotFrame.getX()) / plotFrame.getWidth();
        bandEditors[draggingBand]->setFrequency (getFrequencyForPosition (pos));
        if (draggingGain)
            bandEditors[draggingBand]->setGain (
                getGainForPosition (e.position.getY(),
                                    float (plotFrame.getY()),
                                    float (plotFrame.getBottom())));
    }
}

void FrequalizerAudioProcessorEditor::mouseDoubleClick (
        ...
        for (size_t i = 0; i < size_t (bandEditors.size()); ++i)
        {
            if (auto* band = freqProcessor.getBand (i))
            {
                    ...
                    < clickRadius)
                {
                    if (auto* param =
                            freqProcessor.getPluginState().getParameter (
                                freqProcessor.getActiveParamName (i)))
                        param->setValueNotifyingHost (
                            param->getValue() < 0.5f ? 1.0f : 0.0f);
                }
            }
        }
    }
...
{
    auto pixelsPerDouble =
        2.0f * plotFrame.getHeight() / juce::Decibels::decibelsToGain (maxDB);
    ...
    for (int i = 0; i < bandEditors.size(); ++i)
    {
        ...
        if (auto* band = freqProcessor.getBand (size_t (i)))
        {
            ...
            freqProcessor.createFrequencyPlot (
                ...
                band->magnitudes,
                plotFrame.withX (plotFrame.getX() + 1),
                pixelsPerDouble);
        }
        bandEditor->updateSoloState (freqProcessor.getBandSolo (i));
    }
    frequencyResponse.clear();
    freqProcessor.createFrequencyPlot (frequencyResponse,
                                       freqProcessor.getMagnitudes(),
                                       plotFrame,
                                       pixelsPerDouble);
}
...
float FrequalizerAudioProcessorEditor::getPositionForGain (float gain,
                                                           float top,
                                                           float bottom)
{
    return juce::jmap (juce::Decibels::gainToDecibels (gain, -maxDB),
                       -maxDB,
                       maxDB,
                       bottom,
                       top);
...
float FrequalizerAudioProcessorEditor::getGainForPosition (float pos,
                                                           float top,
                                                           float bottom)
{
    return juce::Decibels::decibelsToGain (
        juce::jmap (pos, bottom, top, -maxDB, maxDB), -maxDB);
}

//==============================================================================
FrequalizerAudioProcessorEditor::BandEditor::BandEditor (
    size_t i,
    FrequalizerAudioProcessor& p)
    : index (i), processor (p)
{
    ...
    frame.setTextLabelPosition (juce::Justification::centred);
    frame.setColour (juce::GroupComponent::textColourId,
                     processor.getBandColour (index));
    frame.setColour (juce::GroupComponent::outlineColourId,
                     processor.getBandColour (index));
    addAndMakeVisible (frame);

    if (auto* choiceParameter = dynamic_cast<juce::AudioParameterChoice*> (
            processor.getPluginState().getParameter (
                processor.getTypeParamName (index))))
    ...
    attachments.add (new juce::AudioProcessorValueTreeState::SliderAttachment (
        processor.getPluginState(),
        processor.getFrequencyParamName (index),
        frequency));
    ...
    attachments.add (new juce::AudioProcessorValueTreeState::SliderAttachment (
        processor.getPluginState(),
        processor.getQualityParamName (index),
        quality));
    ...
    activate.setColour (juce::TextButton::buttonOnColourId,
                        juce::Colours::green);
    buttonAttachments.add (
        new juce::AudioProcessorValueTreeState::ButtonAttachment (
            processor.getPluginState(),
            processor.getActiveParamName (index),
            activate));
...
void FrequalizerAudioProcessorEditor::BandEditor::updateControls (
    FrequalizerAudioProcessor::FilterType type)
{
    switch (type)
    {
...
void FrequalizerAudioProcessorEditor::BandEditor::buttonClicked (
    juce::Button* b)
{
    if (b == &solo)
    {
